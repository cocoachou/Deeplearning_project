...
def generate_hashed_filename(output_directory, image_path, extension):
    hash_object = hashlib.md5(image_path.encode())
    hash_name = hash_object.hexdigest()
    filename = f"{hash_name}.{extension}"
...
def crop_faces_from_txt(input_txt_path, output_directory, target_size=(380, 380), min_size=20, scale_factor=0.709):
    ...
    for image_path in image_paths:
        ...
        # 해시된 파일 이름 생성
        output_path = generate_hashed_filename(output_directory, image_path, 'jpg')

        # 이미 전처리된 파일이 존재하는지 확인
        if os.path.exists(output_path):
            print(f"File already processed: {output_path}, skipping...")
            continue
...
output_directory = '/content/gdrive/MyDrive/DL/team_project/datasets/deepfake_1st/new_crop_real'   # 디렉토리는 개별적으로 변경!!!!
